...
const UsersInfo = () => {
  ...
  const handleAdmin = user =>{
    fetch(`https://bistro-boss-server-psi-black.vercel.app/users/admin/${user._id}`,{
      method: 'PATCH'
    })
    .then(res => res.json())
    .then(data => {
  ...
  const handleDelete = (user) => {
    ...
    //     fetch(`https://bistro-boss-server-psi-black.vercel.app/users/${user._id}`, {
    //       method: "DELETE",
    //     })
    //       .then((res) => res.json())
    //       .then((data) => {
  ...
  return (
    <div className="w-full">
      ...
      <h3 className="text-2xl font-semibold text-center mb-2">Total Users: {users.length}</h3>
